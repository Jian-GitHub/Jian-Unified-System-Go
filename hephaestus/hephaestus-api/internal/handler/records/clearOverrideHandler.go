...
func ClearOverrideHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.ClearOverrideReq
		if e := access.Parse(r, &req); e != nil {
			access.Write(w, nil, e)
			return
		}
		l := records.NewClearOverrideLogic(r.Context(), svcCtx)
		out, e := l.ClearOverride(&req)
